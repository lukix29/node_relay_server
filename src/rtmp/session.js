...
const Logger = new ConsoleLogger("RTMP_SESSION");
...
class RtmpSession extends InternalEventHandler {
    constructor(config, socket) {
        ...
        Logger.level = config.rtmpSession.logLevel;
    ...
    // get appInfo() {
    //     let appArr = this.appname.split("?");
    //     let output = {remoteIp: this.ip};
    //     if (this.appname.length > 0) {
    //         output.app = appArr[0];
    //         output.name = appArr[0].split("/").last();
    //     }
    //     if (appArr.length > 1) {
    //         output.query = appArr.last().split("&").map((t) => {
    //             let arr = t.split("=");
    //             return {[arr[0]]: arr[1]}
    //         });
    //     }
    ...
    stop() {
        if (this.isStarting) {
            ...
            Logger.log(`disconnect id=${this.id}`);
    ...
    onSocketClose() {
        // if(DEBUG) console.log('onSocketClose');
        this.stop();
    }

    onSocketError(e) {
        // if(DEBUG) console.log('onSocketError', e);
        this.stop();
    }

    onSocketTimeout() {
        // if(DEBUG) console.log('onSocketTimeout');
        this.stop();
    }
    ...
    rtmpChunkRead(data, p, bytes) {
        // if(DEBUG) console.log('rtmpChunkRead', p, bytes);
    ...
    rtmpPacketParse() {
        ...
        if (this.parserPacket.header.type > RTMP_TYPE_METADATA) {
            Logger.error("packet parse error.", this.parserPacket);
            this.stop();
        }
    }
    ...
    rtmpControlHandler() {
        ...
        switch (this.parserPacket.header.type) {
            case RTMP_TYPE_SET_CHUNK_SIZE:
                ...
                // if(DEBUG) console.debug('set inChunkSize', this.inChunkSize);
                break;
            case RTMP_TYPE_ABORT:
                break;
            case RTMP_TYPE_ACKNOWLEDGEMENT:
            ...
            case RTMP_TYPE_WINDOW_ACKNOWLEDGEMENT_SIZE:
                ...
                // if(DEBUG) console.debug('set ack Size', this.ackSize);
                break;
            case RTMP_TYPE_SET_PEER_BANDWIDTH:
                break;
        }
    ...
    rtmpVideoHandler() {
        ...
        if (codec_id == 7 || codec_id == 12) {
            ...
            if (frame_type == 1 && payload[1] == 0) {
                ...
                //if(DEBUG) console.log(`[rtmp publish] avc sequence header`,this.avcSequenceHeader);
            }
        }
    ...
    rtmpInvokeHandler() {
        ...
        // if(DEBUG) console.log(invokeMessage);
    ...
    onConnect(invokeMessage) {
        ...
        this.appInfo = {
            ...
            query: (qappidx > 1) ? this.appInfo.query = Helpers.arrayToObject(this.appname.substr(qappidx + 1).split("&")) : {}
        };
        //console.log(appArr, this.appInfo);
        let authRes = this.emit(RtmpSessionEvents.PreConnect, this.sessionInfo);
        if (authRes[0] === false) {
            Logger.log(this.id, "Auth Rejected", authRes);
            this.sendStatusMessage(this.playStreamId, "status", "NetStream.Play.Stop", "Authorization required.");
            return;
        }

        if (!this.isStarting) {
            Logger.log(this.playStreamId, "Is already Starting");
            return;
        }
        //console.log(invokeMessage.cmdObj);
        ...
        this.emit(RtmpSessionEvents.PostConnect, this.sessionInfo);
    }
    ...
    onPublish(invokeMessage) {
        ...
        this.emit(RtmpSessionEvents.PrePublish, this.id, this.publishStreamPath, this.publishArgs);
        ...
        //     if (!results) {
        //         if (DEBUG) console.log(`[rtmp publish] Unauthorized. id=${this.id} streamPath=${this.publishStreamPath} streamId=${this.publishStreamId} sign=${this.publishArgs.sign} `);
        ...
        if (context.publishers.has(this.publishStreamPath)) {
            Logger.log(`[publish] Already has a stream. id=${this.id} streamPath=${this.publishStreamPath} streamId=${this.publishStreamId}`);
            this.sendStatusMessage(this.publishStreamId, "error", "NetStream.Publish.BadName", "Stream already publishing");
        } else if (this.isPublishing) {
            Logger.log(`[publish] NetConnection is publishing. id=${this.id} streamPath=${this.publishStreamPath} streamId=${this.publishStreamId}`);
            this.sendStatusMessage(this.publishStreamId, "error", "NetStream.Publish.BadConnection", "Connection already publishing");
        } else {
            Logger.log(`[publish] New stream. id=${this.id} streamPath=${this.publishStreamPath} streamId=${this.publishStreamId}`);
            ...
            this.emit(RtmpSessionEvents.PostPublish, this.id, this.publishStreamPath, this.publishArgs);
        }
    }
    ...
    onPlay(invokeMessage) {
        ...
        this.emit(RtmpSessionEvents.PrePlay, this.id, this.playStreamPath, this.playArgs);
        ...
        if (this.isPlaying) {
            Logger.log(`[play] NetConnection is playing. id=${this.id} streamPath=${this.playStreamPath}  streamId=${this.playStreamId} `);
            this.sendStatusMessage(this.playStreamId, "error", "NetStream.Play.BadConnection", "Connection already playing");
        } else {
            Logger.log(`[play] NetConnection responding playing. id=${this.id} streamPath=${this.playStreamPath}  streamId=${this.playStreamId} `);
            this.respondPlay();
        }
        //console.log("onPlay: ", [this.playStreamPath, context.publishers]);
        if (context.publishers.has(this.playStreamPath)) {
            this.onStartPlay();
        } else {
            Logger.log(`[play] Stream not found. id=${this.id} streamPath=${this.playStreamPath}  streamId=${this.playStreamId}`);
            this.isIdling = true;
    ...
    onStartPlay() {
        ...
        players.add(this.id);
        //console.log("onStartPlay: ",[ this.playStreamPath, publisherId, publisher, players]);
        ...
        this.isIdling = false;
        this.isPlaying = true;
        this.emit(RtmpSessionEvents.PostPlay, this.id, this.playStreamPath, this.playArgs);
        Logger.log(`[play] Join stream. id=${this.id} streamPath=${this.playStreamPath}  streamId=${this.playStreamId} `);
    }
    ...
    onPause(invokeMessage) {
        ...
        Logger.log(`[play] ${d} stream. id=${this.id} streamPath=${this.playStreamPath}  streamId=${this.playStreamId} `);
        if (!this.isPause) {
            this.sendStreamStatus(STREAM_BEGIN, this.playStreamId);
            if (context.publishers.has(this.publishStreamPath)) {
                //fix ckplayer
    ...
    onReceiveAudio(invokeMessage) {
        ...
        Logger.log(`[play] receiveAudio=${this.isReceiveAudio} id=${this.id} `);
    }

    onReceiveVideo(invokeMessage) {
        this.isReceiveVideo = invokeMessage.bool;
        Logger.log(`[play] receiveVideo=${this.isReceiveVideo} id=${this.id} `);
    }
    ...
    onDeleteStream(invokeMessage) {
        if (invokeMessage.streamId == this.playStreamId) {
            ...
            Logger.log(`[play] Close stream. id=${this.id} streamPath=${this.playStreamPath} streamId=${this.playStreamId}`);
            if (this.isStarting) {
        ...
        if (invokeMessage.streamId == this.publishStreamId) {
            if (this.isPublishing) {
                Logger.log(`[publish] Close stream. id=${this.id} streamPath=${this.publishStreamPath} streamId=${this.publishStreamId}`);
                this.emit(RtmpSessionEvents.DonePublish, this.id, this.publishStreamPath, this.publishArgs);
                if (this.isStarting) {
            ...
            this.publishStreamId = 0;
            //console.log("RESET publishStreamPath");
            this.publishStreamPath = "";
        }
    }
}
